function [z,Ec,psic,V0,Vtot,Ef,Ntot,me]=SchrodingerPoisson1D_f(M,Epsi,meff,BSO,BZO,n,dz,Nloops)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
met = M(:,4);               % Mass: M(z)를 구현하기 위해 추가된항.
% Nloop를 100을 넘기게 코딩하면안됩니다. Newton Raphson algorithm 으로 동작하면안됩니다.
%Damping - injection method만 이용해야합니다.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Discretisation %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while nloop<Nloops
    ...
    [Ec,psic] = Schroed1D_FEM_f(z,Vtot,me,n);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    fermime=dot(psic(:,1).*psic(:,1).*dz,me);
    %Fermi level 을 구할때에 Effective mass 는 Ground state를 이용하여 mass의 기대값을
    %구하는 형태로 진행함.
    ro=[];
    for i=1:length(Ec)
        ro( En>Ec(i),i) = e*fermime*m0/(pi*hbar^2);
        ro( En<Ec(i),i) = 0;
    end
